...
for turn in range(num_datagroup):
    ...
    cost = 0
    for k in range(H):
        cost = cost + ca.mtimes([(X_state[:, k]-Xf).T, Q, (X_state[:, k]-Xf)])+ ca.mtimes([U_input[k], R, U_input[k]])
    ...
    for i in range(1, len(t)):
        ...
        # record
        u_hist[i] = MPC_FirstU
        x_hist[i] = CPM.EulerForwardCartpole_virtual(TS,x_hist[i-1],MPC_FirstU)
        cost_hist[i] = sol.value(cost)
        # update
